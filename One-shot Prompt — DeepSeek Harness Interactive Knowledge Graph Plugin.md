...
> **IMPORTANT ARCHITECTURAL RULE FOR AI GENERATION**:
> - `nhan_buoc` MUST ALWAYS be an Architectural Layer in uppercase (e.g. `GATEWAY / INGRESS`, `COMPUTE / CONCURRENCY`, `SECURITY / IDEMPOTENCY`, `STORAGE / ACID DB`, `ASYNC / QUEUE BUFFER`, `CACHE / DISTRIBUTED LOCK`, `EDGE / WAF RATE LIMIT`, `OBSERVABILITY / AUDIT LOG`). NEVER use linear step numbers like "BƯỚC 1 //" or "Step 1:".
> - `edges.nhan` MUST ALWAYS be a Technical Protocol Flow (e.g. `Webhook Timeout Retry`, `Atomic Lock Check`, `ACID Write / Unique Index`, `Async Event Produce`). NEVER prefix edge labels with numeric ordering like "1. ", "2. ".
